"""My choose your own adventure. Inspired by a mobile game that I played like 10 years ago called 'A Dark Room'. There is some redundancy built into screenwrite and generate functions as I intend on updating it in the future and laying some unused code now is preferable."""
...
def generate() -> list:
    """Generates the location of all of the """
    dimensions: int = 9
    returner: list(list) = []
    holder: list(int)
    #The holder variable isnt technically needed, nor is the list of lists. 
    #A list of ints would suffice because there is only one event per row, but setting this up now would make changing that (having more than 1 event per row, which would be needed on a larger board) in the future much easier.
    i: int = 0
    while i < dimensions:
...
def screen_write(locations: list[list], player: list[int]) -> None:
    ...
    #The above only works because there is only one event on each row. Dimensions would need to be added to the variables exchanged between the functions otherwise (or in future versions)
    i: int = 0
    while i < dimensions:
